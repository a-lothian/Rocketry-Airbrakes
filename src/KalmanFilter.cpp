...
void KalmanFilter::Predict(float dt, float a_imu) {
    float dt2 = dt*dt;
    float tmp[3][3];
    float q0, q1, q2; // diagonal of process noise matrix

    // recompute Q with dt
    q0 = stddev_a * stddev_a * (dt2*dt2) / 4.0f;
    q1 = stddev_a * stddev_a * dt2;
    q2 = stddev_b * stddev_b * dt;
    ...
    P[0][1] += stddev_a * stddev_a * (dt2 * dt) / 2.0f;   // Q[0][1]
...
void KalmanFilter::Update(float z_baro) {
